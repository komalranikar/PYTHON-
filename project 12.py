...
mydb=mysql.connector.connect(user='root',passwd='root',host='localhost',database='ki')
...
def Menu(): #Function to display the menu
    ...
    print("6. TransactionsDebit/Withdraw from theaccount".center(140))
...
def Create():
    try:
        mycursor.execute('create table bank (ACCNO int(10),NAME varchar(20),MOBILE int(10),EMAIL varchar(20),ADDRESS varchar(20),CITY varchar(10),COUNTRY varchar(20),BALANCE int(15))')
        print("Table Created")
        Insert()
    except:
        print("Table Exist")
...
def Insert():
    while True: #Loop for accepting records
        print("NOTE : Account number should be of integer type only")
        ...
        if ch=='N' or ch=='n':
            break
def DispSortAcc(): #Function to Display records as perascending order of Account Number
    try:
...
def DispSortName(): #Function to Display records as perascending order of Name
    try:
        ...
        print("Table doesn't exist")
def DispSortBal(): #Function to Display records as perascending order of Balance
    try:
